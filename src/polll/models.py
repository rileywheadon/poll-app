...
class PollAnswer(db.Model):
    ...
    discrete_responses : so.Mapped["DiscreteResponse"] = so.relationship("DiscreteResponse", back_populates = "answer")
    ranked_responses : so.Mapped["RankedResponse"] = so.relationship("RankedResponse", back_populates = "answer")
    tiered_responses : so.Mapped["TieredResponse"] = so.relationship("TieredResponse", back_populates = "answer")
...
class NumericResponse(db.Model):
    id = so.mapped_column(sa.Integer, primary_key = True)
...
class DiscreteResponse(db.Model):
    id = so.mapped_column(sa.Integer, primary_key = True)
